...
class JdgoodslistSpider(scrapy.Spider):
    ...
    def __generateUrl(self, cat, page):
        return 'http://list.jd.com/list.html?cat={0}&ev=exbrand_7817' \
            '&page={1}&delivery=1&stock=0&sort=sort_totalsales15_desc' \
            '&trans=1&JL=4_7_0#J_main'.format(cat, page)
    ...
    def start_requests(self):
        try:
            ...
            for cat in f.readlines():
                ...
                yield scrapy.Request(url, meta={'cat': cat},
                    callback=self.parseGoodsPages)
    ...
    def parseGoodsList(self, response):
        lis = response.xpath(
            '//div[@class="ml-wrap"]/div[@id="plist"]/ul/li[@class="gl-item"]/div')
        for sel in lis:
            item = JdGoodsListItem()
            try:
                item['name'] = sel.xpath(
                    'div[@class="p-name"]/a/em/text()').extract()[0]
                item['url'] = 'http:' + \
                    sel.xpath('div[@class="p-name"]/a/@href').extract()[0]
            except:
